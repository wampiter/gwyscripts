...
for c in cons:
    ...
    if not os.path.exists('%s/manproc' % path):
        os.makedirs('%s/manproc' % path)
    print('Saving %s' % name)
    #I like this palette:
    ...
    gwyutils.save_dfield_to_png(c, key, '%s/manproc/%s.png' % (path,name), gwy.RUN_NONINTERACTIVE)
    gwy_file_save(c, '%s/manproc/%s.gwy' % (path,name), gwy.RUN_NONINTERACTIVE)
